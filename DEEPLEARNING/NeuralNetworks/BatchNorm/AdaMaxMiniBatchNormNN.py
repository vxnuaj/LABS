...
def update(w2, g2, b2, w1, g1, b1, dw2, dg2, db2, dw1, dg1, db1, sdw1, sdg1, sdb1, sdw2, sdg2, sdb2, vdw1, vdg1, vdb1, vdw2, vdg2, vdb2, alpha, beta1, beta2, epoch):

    eps = 1e-8

    '''
    COMPUTING EWA OF GRADIENTS 
    '''
    ...
    '''
    COMPTUING EWA BASED ON PTH RAISED GRADIENTS    
    '''

    '''
    Per the Adam Paper, this turns out to be numerically unstable. I myself empirically get these numerically unstable results.

    https://arxiv.org/pdf/1412.6980

    "In Adam, the update rule for individual weights is to scale their gradients inversely proportional to a (scaled) L
    2 norm of their individual current and past gradients. We can generalize the L2 norm based update rule to a Lp norm 
    based update rule. Such variants become numerically unstable for large p."

    Turns out, you get a type of exploding gradient that then explodes the inputs to the softmax, thereby causing numerical overflow, at least that was the case in my scenario
    
    sdw1 = ((beta2 ** (epoch + 1)) * sdw1) + (1 - (beta2 ** (epoch + 1))) * np.power(np.abs(dw1), epoch + 1)
    sdg1 = ((beta2 ** (epoch + 1)) * sdg1) + (1 - (beta2 ** (epoch + 1))) * np.power(np.abs(dg1), epoch + 1)
    sdb1 = ((beta2 ** (epoch + 1)) * sdb1) + (1 - (beta2 ** (epoch + 1))) * np.power(np.abs(db1), epoch + 1)
    
    sdw2 = ((beta2 ** (epoch + 1)) * sdw2) + (1 - (beta2 ** (epoch + 1))) * np.power(np.abs(dw2), epoch + 1)
    sdg2 = ((beta2 ** (epoch + 1)) * sdg2) + (1 - (beta2 ** (epoch + 1))) * np.power(np.abs(dg2), epoch + 1)
    sdb2 = ((beta2 ** (epoch + 1)) * sdb2) + (1 - (beta2 ** (epoch + 1))) * np.power(np.abs(db2), epoch + 1)
    
    w2 = w2 - (alpha / (np.power(sdw2 + eps, (1 / (epoch + 1)) ))) * vdw2
    g2 = g2 - (alpha / (np.power(sdg2 + eps, (1 / (epoch + 1))))) * vdg2
    b2 = b2 -  (alpha / (np.power(sdb2 + eps, ( 1 / (epoch + 1)) ))) * vdb2
    w1 = w1 -  (alpha / (np.power(sdw1 + eps, (1 / (epoch + 1))))) * vdw1
    g1 = g1 -  (alpha / (np.power(sdg1 + eps, (1 / (epoch + 1))))) * vdg1
    b1 = b1 -  (alpha / (np.power(sdb1 + eps, ( 1 / (epoch + 1))))) * vdb1

    So then we can take the derivation of AdaMax's computation and simplify it as (below) to mitigate numerical overflow. (see paper for derivation)
    '''

    sdw1 = np.maximum(beta2 * sdw1, np.abs(dw1))
    sdg1 = np.maximum(beta2 * sdg1, np.abs(dg1))
    sdb1 = np.maximum(beta2 * sdb1, np.abs(db1))

    sdw2= np.maximum(beta2 * sdw2, np.abs(dw2))
    sdg2 = np.maximum(beta2 * sdg2, np.abs(dg2))
    sdb2 = np.maximum(beta2 * sdb2, np.abs(db2))

    '''
    ADAMAX UPDATE RULE
    '''

    w2 = w2 - (alpha / (sdw2)) * vdw2
    g2 = g2 - (alpha / (sdg2)) * vdg2
    b2 = b2 -  (alpha / (sdb2)) * vdb2
    w1 = w1 -  (alpha / (sdw1)) * vdw1
    g1 = g1 -  (alpha / (sdg1)) * vdg1
    b1 = b1 -  (alpha / (sdb1)) * vdb1
...
def gradient_descent(x, y, w1, g1, b1, w2, g2, b2, epochs, alpha, beta1, beta2, batch_num):
    ...
    for epoch in range(epochs):
        for i in range(x.shape[0]):
            ...
            w2, g2, b2, w1, g1, b1, sdw1, sdg1, sdb1, sdw2, sdg2, sdb2, vdw1, vdg1, vdb1, vdw2, vdg2, vdb2, = update(w2, g2, b2, w1, g1, b1, dw2, dg2, db2, dw1, dg1, db1, sdw1, sdg1, sdb1, sdw2, sdg2, sdb2, vdw1, vdg1, vdb1, vdw2, vdg2, vdb2, alpha, beta1, beta2, epoch)
