...
def update(w2, g2, b2, w1, g1, b1, dw2, dg2, db2, dw1, dg1, db1, sdw1, sdg1, sdb1, sdw2, sdg2, sdb2, alpha, beta):
    ...
    sdg1 = (beta * sdg1) + (1 - beta) * np.square(dg1)
    sdb1 = (beta * sdb1) + (1 - beta) * np.square(db1)
    
    sdw2 = (beta * sdw2) + (1 - beta) * np.square(dw2)
    sdg2 = (beta * sdg2) + (1 - beta) * np.square(dg2)
    ...
    w2 = w2 - (alpha / (np.sqrt(sdw2 + eps))) * dw2
    g2 = g2 - (alpha / (np.sqrt(sdg2 + eps))) * dg2
    b2 = b2 -  (alpha / (np.sqrt(sdb2 + eps))) * db2
    w1 = w1 -  (alpha / (np.sqrt(sdw1 + eps))) * dw1
    g1 = g1 -  (alpha / (np.sqrt(sdg1 + eps))) * dg1
    b1 = b1 -  (alpha / (np.sqrt(sdb1 + eps))) * db1
    return w2, g2, b2, w1, g1, b1, sdw1, sdg1, sdb1, sdw2, sdg2, sdb2

def gradient_descent(x, y, w1, g1, b1, w2, g2, b2, epochs, alpha, beta, batch_num):
    ...
    sdw1, sdg1, sdb1, sdw2, sdg2, sdb2 = 0, 0, 0, 0, 0, 0 # initializing EWAs to 0, given no history
    ...
    for epoch in range(epochs):
        for i in range(x.shape[0]):
            ...
            w2, g2, b2, w1, g1, b1, sdw1, sdg1, sdb1, sdw2, sdg2, sdb2 = update(w2, g2, b2, w1, g1, b1, dw2, dg2, db2, dw1, dg1, db1, sdw1, sdg1, sdb1, sdw2, sdg2, sdb2, alpha, beta)
...
def model(x, y, epochs, alpha, beta, batch_num):
    w1, g1, b1, w2, g2, b2 = init_params()
    w1, g1, b1, w2, g2, b2, acc_vec, loss_vec, epoch_vec = gradient_descent(x, y, w1, g1, b1, w2, g2, b2, epochs, alpha, beta, batch_num)
    return w1, g1, b1, w2, g2, b2, acc_vec, loss_vec, epoch_vec
...
if __name__ == "__main__":
    ...
    w1, g1, b1, w2, g2, b2, acc_vec, loss_vec, epoch_vec = model(X_train, Y_train, epochs, alpha, beta, batch_num)
